...
Aynı kaynağın her arka uçta aynı sonucu vermesi için:

### 1. Tam sayı bölmesinde `/` kullanma, `tamBol()` kullan

`/` işleci arka uçlar arasında **farklı davranır**:

| İfade | Yorumlayıcı / C yolu | `elf` / `asm` yolu |
|---|---|---|
| `1000 / 256` | `3.90625` | `3` |
...
```tan
yaz(tamBol(1000, 256))   # her arka uçta 3
```

`tamBol` sıfıra doğru keser (C/Go/idiv semantiği). `%` işleci zaten her yerde
tutarlıdır.

### 2. İkili dosya yazarken `yazBaytlar()` kullan

Metin üzerinden ham bayt yazma **çalışmaz**: yorumlayıcıda metin bir rune
dizisidir, `karakter(200)` UTF-8'de **iki bayt** üretir. `elf` arka ucunda
metin bayt dizisidir, aynı çağrı **tek bayt** verir.

```tan
# YANLIŞ — arka uçlar farklı dosya üretir
yaz_dosya("cikti.bin", baytListesindenMetin(baytlar))

# DOĞRU — her arka uçta aynı
yazBaytlar("cikti.bin", baytlar)
```

### 3. Dosya adları ASCII olmalı

Dosya adlarında `İ`, `Ş`, `Ğ`, `Ü`, `Ö`, `Ç` kullanma. Türkçe büyük harf
kuralı **kod içindeki tanımlayıcılar** için geçerlidir, dosya adları için
değil. `Istatistik.tan` doğru, `İstatistik.tan` yanlış — ikincisi git,
URL ve farklı dosya sistemlerinde sorun çıkarır.

### 4. Windows/WSL'de çalışıyorsan

- Dosya adı değiştirirken büyük/küçük harf duyarsız dosya sistemi seni
  yanıltır: `temel.tan` ve `Temel.tan` orada aynı dosyadır, Linux'ta ve
  GitHub'da değildir.
- Satır sonlarını LF tut. Repoda `.gitattributes` bunu zorluyor.

### 5. Fark testi

Bir programın arka uçlar arası tutarlılığını sınamak için:

```bash
./tan elf X.tan A     # elf yolu
./tan X.tan           # yorumlayıcı yolu
cmp A B               # birebir aynı olmalı
```
